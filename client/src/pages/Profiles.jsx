...
const Profiles = () => {
  ...
  const handleAddDietary = () => {
    ...
    setForm((prev) => ({
      ...prev,
      dietaryRestrictions: [...prev.dietaryRestrictions, trimmed],
    }));
    setDietaryInput("");
  };
  ...
  const handleAddMedication = () => {
    ...
    if (!trimmed) return;
    if (
      form.medications.some((m) => m.toLowerCase() === trimmed.toLowerCase())
    ) {
      setMedicationInput("");
      return;
    }
  ...
  const handleDelete = async (dep) => {
    const confirmed = window.confirm(
      `Delete ${dep.name}'s profile? This cannot be undone.`,
    );
    if (!confirmed) return;
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-300 mt-1">
            Update your own allergy/diet/medication info anytime, and manage
            profiles for anyone else you scan food for.
          </p>
        </div>
        {!formOpen && (
          <button
      ...
      {formOpen && (
        <div className="card mb-8 border border-primary-200">
          ...
          <div className="mb-6">
                ...
                <button
                  ...
                  className={`px-3 py-1.5 text-sm rounded-full border-2 transition-all ${
                    form.allergies.includes(a)
                      ...
                      : "border-gray-200 hover:border-red-300 text-gray-600 dark:text-gray-300"
                  }`}
                >
                  {form.allergies.includes(a) && (
            ...
            {form.allergies.filter((a) => !commonAllergens.includes(a)).length >
              0 && (
          ...
          <div className="mb-6">
                ...
                <button
                  ...
                  className={`px-3 py-1.5 text-sm rounded-full border-2 transition-all ${
                    form.dietaryRestrictions.includes(d)
                      ...
                      : "border-gray-200 hover:border-primary-300 text-gray-600 dark:text-gray-300"
                  }`}
                >
                  {form.dietaryRestrictions.includes(d) && (
                    <FiCheck className="inline mr-1" />
            ...
            {form.dietaryRestrictions.filter((d) => !dietaryOptions.includes(d))
              .length > 0 && (
              <div className="flex flex-wrap gap-2">
          ...
          <div className="flex gap-3 justify-end">
            <button
              onClick={closeForm}
              className="px-4 py-2 text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-gray-100"
            >
              Cancel
            </button>
            <button
          ...
          <div className="card mb-8 border-2 border-primary-100">
            ...
            {user?.preferences?.medications?.length > 0 && (
              <div>
                ...
                <span className="text-sm text-gray-700 dark:text-gray-200">
                  {user.preferences.medications.join(", ")}
                </span>
              </div>
            )}
            {!user?.preferences?.allergies?.length &&
              !user?.preferences?.dietaryRestrictions?.length &&
              !user?.preferences?.medications?.length && (
                <p className="text-sm text-gray-400 dark:text-gray-500">
                  No allergies, dietary restrictions, or medications on file yet
                  — click the edit icon to add some.
                </p>
              )}
          </div>
        </>
        ...
        <>
          ...
          {loadingDependents ? (
            ...
            <div className="text-center py-12 text-gray-500 dark:text-gray-400">
              <FiUser className="text-4xl mx-auto mb-3 text-gray-300 dark:text-gray-500" />
              <p className="mb-1">No family profiles yet</p>
              <p className="text-sm">
                Add one to start scanning food for someone else's allergies or
                diet.
              </p>
            </div>
          ) : (
                ...
                <div key={dep._id} className="card">
                  ...
                  {dep.preferences?.allergies?.length > 0 && (
                    <div className="mb-2">
                      ...
                      <span className="text-sm text-gray-700 dark:text-gray-200">
                        {dep.preferences.allergies.join(", ")}
                      </span>
                    </div>
                  )}
                  {dep.preferences?.dietaryRestrictions?.length > 0 && (
                    <div className="mb-2">
                      ...
                      <span className="text-sm text-gray-700 dark:text-gray-200">
                        {dep.preferences.dietaryRestrictions.join(", ")}
                      </span>
                    </div>
                  )}
                  {dep.preferences?.medications?.length > 0 && (
                    <div>
                      ...
                      <span className="text-sm text-gray-700 dark:text-gray-200">
                        {dep.preferences.medications.join(", ")}
                      </span>
                    </div>
                  )}
                  {!dep.preferences?.allergies?.length &&
                    !dep.preferences?.dietaryRestrictions?.length &&
                    !dep.preferences?.medications?.length && (
                      <p className="text-sm text-gray-400 dark:text-gray-500">
